@if (isset($permissions['CHANGE_STATUS']) && hasPermission($permissions['CHANGE_STATUS']))
    <div class="mx-1 dropdown">
        <a href="#" class="d-flex align-items-center text-dark text-decoration-none dropdown-toggle"
            id="statusDropDown" data-bs-toggle="dropdown" aria-expanded="true">
        
            <span
                class="badge bg-{{$status['bt_class'][$status['current_status']]}}">
                ...
                <li >
                    <a class="dropdown-item"
                        href="{{ route($tenantRoute . $module . '.changeStatus', ['id' => $id,'status' =>$availableStatus ]) }}">
                          <span class="status-circle bg-{{$status['bt_class'][$availableStatus]}} me-2"></span>  {{ $availableStatus }}
                    </a>
                </li>
