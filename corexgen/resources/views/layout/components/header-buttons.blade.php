        ...
        <div class="d-flex">
            ...
            @if (isset($permissions['KANBAN_BOARD']) &&
                ...
                @elseif (Route::currentRouteName() === getPanelRoutes($module . '.index'))
                    ...
                    <a class="btn btn-outline-secondary btn-xl me-2" data-toggle="tooltip"
                        href="{{ route(getPanelRoutes($module . '.kanban')) }}" title="{{ __('crud.Kanban Board') }}">
...
@push('scripts')
    <script>
        function toggleStats() {
            ...
            const button = $('#toggle-stats-btn');

            // Toggle a class to indicate active/inactive state
            if (button.hasClass('active')) {
                button.removeClass('active');
                // Optionally, change the tooltip or button text
                button.attr('title', 'Show Stats').tooltip('update');
            } else {
                button.addClass('active');
                button.attr('title', 'Hide Stats').tooltip('update');
            }
        }
    </script>
@endpush
