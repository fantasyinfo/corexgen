@php
    // prePrintR($proposal->product_details);
@endphp
...
<input type="hidden" name="total_amount" value="1" id="total_amount" />
@push('scripts')
    <script>
        $(document).ready(function() {
            const products = @json($products);
            const taxOptions = @json($tax);
            const proposalInJson = @json($invoice);

            // Get product details directly from proposal JSON
            const productDetails = JSON.parse(proposalInJson?.product_details)?.products || [];
            const additionalFields = JSON.parse(proposalInJson?.product_details)?.additional_fields || {
                discount: 0,
                adjustment: 0
            };
