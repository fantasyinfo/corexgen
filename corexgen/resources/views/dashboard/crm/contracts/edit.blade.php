...
@section('content')
    @php
        // prePrintR($contract->toArray());
        $type = $contract->typable_type === App\Models\CRM\CRMClients::class ? 'client' : 'lead';
    @endphp
        ...
        <div class="row">
            <div class="justify-content-md-center col-lg-12">
                <div class="card stretch stretch-full">
                    <form id="proposalsFieldsForm" action="{{ route(getPanelRoutes('contracts.update')) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="card-body general-info">
                            <div class="mb-5 d-flex align-items-center justify-content-between">
                                <p class="fw-bold mb-0 me-4">
                                    <span class="d-block">{{ __('contracts.Update Contract') }}</span>
                                ...
                                <div>
                                    <button type="submit" class="btn btn-primary">
                                        <i class="fas fa-save"></i> {{ __('contracts.Update Contract') }}
                                    </button>
                                </div>
                            </div>
                            ...
                            <div id="clientSection"
                                style="display:  {{ old('type', $contract->typable_type) === App\Models\CRM\CRMClients::class ? 'flex' : 'none' }};">
                                <div class="col-lg-4">
                                    ...
                                    <select name="client_id" id="client_id" class="form-select searchSelectBox">
                                        @foreach ($clients as $item)
                                            ...
                                            <option value="{{ $item->id }}"
                                                {{ old('client_id', $contract->typable_id) == $item->id ? 'selected' : '' }}>
                                                {{ $nameAndEmail }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div id="leadSection"
                                style="display:  {{ old('type', $contract->typable_type) === App\Models\CRM\CRMLeads::class ? 'flex' : 'none' }};">
                                <div class="col-lg-4">
                                ...
                                <div class="col-lg-8">
                                    ...
                                    <select name="lead_id" id="lead_id" class="form-select searchSelectBox">
                                        @foreach ($leads as $item)
                                            ...
                                            @endphp
                                            <option value="{{ $item->id }}"
                                                {{ old('lead_id', $contract->typable_id) == $item->id ? 'selected' : '' }}>
                                                {{ $nameAndEmail }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="row mb-4 align-items-center">
                                ...
                                <div class="col-lg-8">
                                    <x-form-components.input-group type="text" class="custom-class" id="_prefix"
                                        ...
                                        value="{{ old('_prefix', $contract->_prefix) }}" disabled="true" required
                                ...
                                <div class="col-lg-8">
                                    <x-form-components.input-group-prepend-append type="number" class="custom-class"
                                        ...
                                        append="{{ getSettingValue('Currency Code') }}"
                                         name="value"
                                        placeholder="{{ __('99999') }}" value="{{ old('value', $contract->value) }}" />
                            ...
                            <div class="row mb-3 align-items-center">
                                ...
                                <div class="col-lg-8">
                                    <x-form-components.create-new :link="'contracts.createContracts'" :text="'Create new'" />
                                    <select name="template_id" id="template_id" class="form-select">
                                        <option value="">Select Contract Template (optional)</option>
                                        @foreach ($templates as $item)
                                            <option value="{{ $item->id }}"
                                                {{ old('template_id', $contract->template_id) == $item->id ? 'selected' : '' }}>
                                                {{ $item->title }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="row mb-4 align-items-center">
                                ...
                                <x-form-components.textarea-group name="details" id="details"
                                    ...
                                    value="{{ old('details', $contract->details) }}" class="custom-class details" />
                ...
                tinymce.init({
                    ...
                    setup: function(editor) {
                        editor.on('init', function() {
                            editor.setContent(`{!! $contract->details !!}`);
                        });
                    },
                    menubar: true,
                    plugins: [
