    ...
    <div class="row">
        <div class="col-lg-9">
            <div class="card stretch stretch-full">
                ...
                <form id="userForm" action="{{ route(getPanelRoutes('tax.update')) }}" method="POST">
                    @csrf
                    @method('PUT')
                                    ...
                                    <select class="form-control searchSelectBox select2-hidden-accessible @error('country_id') is-invalid @enderror" name="country_id" id="country_id">
                                        @if($countries && $countries->isNotEmpty())
                                            @foreach($countries as $countries)
                                                <option value="{{ $countries->id }}" {{ $tax->country_id == $countries->id ? 'selected' : '' }}>
                                                    {{ $countries->name }}
                                                </option>
                                            @endforeach
                                        @else
